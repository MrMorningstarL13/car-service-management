...
export default function RepairCard({ repair, employees, onAssignEmployee }: RepairCardProps) {
  ...
  const getServiceTypeName = (serviceTypeId: number) => {
    ...
    return serviceTypes[serviceTypeId] || `Service Type ${serviceTypeId}`
  }
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="font-medium text-gray-900">{getServiceTypeName(repair.serviceTypeId)}</h4>
        </div>
        <span
          className={`px-2 py-1 text-xs rounded-full ${
            repair.isComplete ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
          }`}
        >
